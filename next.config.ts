...
const securityHeaders = [
  ...
  // Uploads use short-lived signed URLs generated for the private GCS bucket.
  // Keep connections restricted, allowing only the exact Google Storage host
  // required for PUT uploads and signed media operations.
  { key: "Content-Security-Policy", value: "default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'none'; form-action 'self'; img-src 'self' data: blob: https:; font-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; connect-src 'self' https://storage.googleapis.com; frame-src https://www.youtube-nocookie.com https://www.youtube.com; upgrade-insecure-requests" }
];
