...
export function FileUpload({ courseId, inputName, kind, existingUrl, target = "course" }: { courseId?: string; inputName: string; kind: "document" | "video" | "image"; existingUrl?: string | null; target?: "course" | "organization_logo" | "profile_avatar" }) {
  ...
  return <div className="uploadDropzone" aria-busy={uploading}><input name={inputName} type="hidden" value={url} /><input aria-label={t.choose} accept={accept} className="srOnly" onChange={(event) => event.target.files?.[0] && upload(event.target.files[0])} ref={input} type="file" /><button className="buttonGhost" disabled={uploading} onClick={() => input.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const file = event.dataTransfer.files[0]; if (file) upload(file); }} type="button">{uploading ? t.sending : t.choose}</button><p aria-live="polite" role="status" className="formHint">{limit} {message || (url ? t.linked : "")}</p></div>;
}
