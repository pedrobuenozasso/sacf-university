...
export async function POST(request: Request) {
  ...
  const body = await request.json() as { courseId?: string; name?: string; type?: string; size?: number; kind?: "document" | "video" | "image" };
  const size = body.size;
  if (!body.courseId || !body.name || !body.type || !body.kind || typeof size !== "number" || !Number.isFinite(size) || size <= 0 || body.name.length > 255 || body.type.length > 120) return NextResponse.json({ error: "invalid_request" }, { status: 400 });
