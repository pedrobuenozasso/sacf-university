  ...
  return (
    <div className="lessonEditor">
      ...
      <section className="lessonEditorLayout">
        <form className="detailPanel lessonContentPanel" action={updateLesson}>
        ...
        <aside className="detailPanel lessonAssessmentPanel">
          ...
          {lesson.lessonType !== "quiz" ? <p className="formHint">Altere o tipo da aula para “Prova” e salve para cadastrar questões.</p> : <>
            {lesson.questions.map((question, index) => <div className="moduleItem quizQuestionCard" key={question.id}><div className="moduleCardHeader"><strong>{index + 1}. {question.question}</strong><form action={deleteQuizQuestion}><input name="courseId" type="hidden" value={courseId} /><input name="lessonId" type="hidden" value={lesson.id} /><input name="questionId" type="hidden" value={question.id} /><button className="dangerButton" type="submit">Excluir</button></form></div><ul>{question.options.map((option) => <li key={option.id}>{option.isCorrect ? "✓ " : ""}{option.optionText}</li>)}</ul></div>)}
